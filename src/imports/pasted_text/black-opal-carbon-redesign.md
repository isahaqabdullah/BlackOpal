Project
Redesign the Black Opal Carbon website as a modern industrial B2B manufacturing site for activated carbon products.
Brand position
Black Opal Carbon is a manufacturer/supplier of coconut shell activated carbon serving industrial and water treatment use cases. The brand should feel:
technical
credible
clean
global
industrial
premium but practical
Audience
Primary users:
procurement managers
engineers
distributors
water treatment buyers
industrial operations buyers
OEM/filter partners
Main goals
Communicate trust and manufacturing scale fast
Make product categories easy to understand
Show certifications and technical credibility
Help buyers identify the right application/product match
Drive quote requests and technical inquiries
Visual direction
Minimal, clean industrial aesthetic
Modern typography with strong hierarchy
White/light background with charcoal, deep blue, muted green accents
Use product/media imagery more selectively
Strong card-based layout for products and applications
Use iconography for certifications, scale, warehouses, and industries
Premium B2B feel, not template-like, not crowded
Homepage structure
1. Hero section
Headline:
“Coconut Shell Activated Carbon for Water, Air, Gas, and Industrial Applications”
Subheadline:
“High-performance activated carbon solutions backed by large-scale manufacturing, certified quality, and application-specific expertise.”

CTAs:

Request a Quote
Talk to Technical Sales
Hero supporting proof:
NSF 42 / NSF 61
Prop 65 Compliant
50 million lbs capacity
2 production centers
U.S. headquarters + warehouse network
2. Trust bar
Display:
Certifications
Manufacturing scale
Year established
Warehouses / logistics footprint
3. Product categories
Four cards:
Granular Activated Carbon
Powder Activated Carbon
Impregnated Activated Carbon
Catalytic Activated Carbon
Each card should include:
short benefit summary
key use cases
button to view product details
4. Applications section
Grid of industries:
Water Treatment
Gold Recovery
Air & Gas
Oil & Gas
Catalytic / Chloramine Removal
Other Industrial Applications
5. Why Black Opal Carbon
Three to six blocks:
Coconut shell specialization
Consistent manufacturing quality
Application-specific product grades
Warehousing / supply reliability
Technical support
Certifications and compliance
6. Featured product / flagship capability
Highlight catalytic carbon and chloramine removal, since the current site gives that unusually strong emphasis.
7. Company / manufacturing section
Short story:

established in 2010
joint venture roots
manufacturing facilities in India
U.S. headquarters in Pittsburgh
reliability, consistency, service
8. Client / partner logos
Cleaner monochrome logo grid with better spacing
9. CTA band
“Need the right activated carbon for your process?”
Buttons:

Request Quote
Send Technical Inquiry
10. Footer
Products
Applications
About
Newsroom
Contact
phone
email
headquarters
warehouse locations
Suggested sitemap for the rebuild
Instead of one overloaded homepage, split it into:
Home
Products
Granular Activated Carbon
Powder Activated Carbon
Impregnated Activated Carbon
Catalytic Activated Carbon
Applications
Water Treatment
Gold Recovery
Air & Gas
Oil & Gas
Other Applications
About
Production / Manufacturing
Certifications / Quality
Newsroom
Contact / Request Quote
This is more aligned with the current content than the present single-page density.
Copy direction
The current copy is informative but long and sometimes repetitive. A good rewrite should move toward:
shorter headlines
clearer benefit-first language
technical detail below the fold
fewer paragraphs, more structured bullets/spec blocks
stronger conversion language
Example tone:
“Engineered for chloramine reduction”
“Built for consistent adsorption performance”
“Reliable supply from large-scale coconut shell carbon manufacturing”
“Application-matched grades for water, gas, and industrial treatment”
That keeps the technical seriousness while sounding current.
What to keep from the original content
Keep these facts visible in the redesign because they are strong trust anchors from the current site:
Coconut activated carbon focus
NSF 42 / NSF 61 references
Prop 65 compliant messaging
Product family breakdown
Application sectors
Established 2010 / 2 production centers / 4 warehouse locations / 50 million lbs
Pittsburgh HQ and contact details
What to improve or add
The current site would benefit from pages or modules for:
downloadable technical data sheets
clearer product comparison tables
request-a-quote flow with use-case fields
certifications page
manufacturing/process page
FAQ for application selection
stronger mobile responsiveness
better forms and lead capture
These are not explicitly on the current site, but they would make the redesign materially better for B2B conversion.
One compact prompt for Figma AI
“Design a modern industrial B2B website for Black Opal Carbon, a coconut shell activated carbon manufacturer. Create a clean, premium, technical homepage with strong trust signals, modern typography, card-based sections, and conversion-focused CTAs. Include hero, certifications bar, product category cards, application grid, manufacturing credibility section, client logos, and contact CTA. Style should feel credible, global, industrial, and minimal, using white, charcoal, deep blue, and muted green. Audience is procurement managers, engineers, and industrial buyers. Prioritize clarity, technical trust, and quote generation.”
Content summary for the designer
Company sells coconut shell activated carbon
Main products: granular, powder, impregnated, catalytic
Main applications: water treatment, gold recovery, air & gas, oil & gas, catalytic/chloramine removal
Trust points: NSF 42, NSF 61, Prop 65 compliant
Scale points: established 2010, 2 production centers, 4 warehouses, 50 million lbs
U.S. HQ in Pittsburgh, with multiple warehouse locations
Primary CTA should be quote/inquiry driven
I can turn this next into a full page-by-page wireframe outline or actual rewritten website copy for the new design.






