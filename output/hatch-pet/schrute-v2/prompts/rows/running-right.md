Create a single horizontal sprite strip for the Codex app digital pet `schrute` in the state `running-right`.
...
- Make this a determined right-facing power-walk/march rather than a cute jog.
- Keep the face serious, the tie slightly angled by motion, and the beet clutched like mission cargo.
- Use stronger leg and arm positions while keeping one complete pose centered in each slot.
