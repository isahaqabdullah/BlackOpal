Create a single horizontal sprite strip for the Codex app digital pet `schrute` in the state `waving`.
...
- Make the greeting a stiff salute that awkwardly becomes a tiny half-wave, then snaps back to serious posture.
- Do not make it cute or bouncy; the humor should come from how formal and over-serious the gesture is.
- Show the gesture only through arm and paw pose.
