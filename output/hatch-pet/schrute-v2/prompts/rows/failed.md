Create a single horizontal sprite strip for the Codex app digital pet `schrute` in the state `failed`.
...
Schrute v2 action direction:
- Make failure read as wounded dignity: glasses slip, tie droops, shoulders collapse, beet lowers sadly, then he tries to regain composure.
- Keep any tears or stars attached to the pet if used, but prefer posture and expression over effects.
- Do not add red X marks, floating symbols, loose smoke, or disconnected effects.
