Create a single horizontal sprite strip for the Codex app digital pet `schrute` in the state `idle`.
...
- Make idle read as rigid assistant-manager vigilance, not a soft breathing loop.
- Frame sequence should include a suspicious squint/blink, a tiny tie-straighten, a beet held like official evidence, and a return to stiff posture.
- Keep the movement subtle but unmistakably intense.
