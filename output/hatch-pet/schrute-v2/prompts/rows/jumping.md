Create a single horizontal sprite strip for the Codex app digital pet `schrute` in the state `jumping`.
...
- Make the jump a controlled victory hop, as if Schrute won a tiny workplace inspection.
- Include a clenched-fist or beet-raised peak pose, then a disciplined landing back into posture.
- Show height through body position only, with no shadows, dust, landing marks, or motion effects.
